...
public class PuzzlePanel extends JPanel {
	...
	public PuzzlePanel(Schiebepuzzle parent, int n) {
		...
		/* *** Aufgabenteil (1b) *** */
		// 1P
		PuzzleListener pListener = new PuzzleListener(this);
		int count = 1;
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < n; j++) {
				if ((i < (n - 1)) || (j < (n - 1))) {
					this.teile[i][j] = new ZahlButton(count++);
					this.teile[i][j].addActionListener(pListener);
				} else {
	 ...
	 * ein Update der Oberflaeche (nach einem Zug oder fuer ein neues Spiel)
	 * durchfuehren.
	 */
